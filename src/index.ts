...
// Expose per-request ingress path (HA sets this header) to templates
app.use((req, res, next) => {
  const headerPath = req.get("X-Ingress-Path");
  res.locals.ingressPath = typeof headerPath === "string" ? headerPath : "";
  next();
});
